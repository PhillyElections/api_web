...
class ShapeCityDivision
{
    ...
    public function __construct($queried)
    {
        ...
        $this->table_name = '`shapes_city_divisions`';
        $this->queried_index = '`division_n`';
    }
        ...
        if ($this->queried) {
            ...
            if ($stmt->execute()) {
                ...
                if (count($data)) {
                    ...
                    $features[0]['geometry']['coordinates'][0] = $data[0]['SHAPE'];//$utils->polygonString2Array($data[0]['SHAPE']);
                    $status = 'success';
                }
            }
        }
